...
- Shared workbench network resources, owner-scoped IAM identities, and per-developer workbench instances are managed by separate Terraform roots and workflows.
- Workbench network state is shared; IAM identity state is split by owner; developer instance state is split by owner and AZ slot.
...
- Snowflake workload identity federation for workbench execution uses service users mapped to owner-scoped AWS IAM roles; human PERSON users remain separate.
