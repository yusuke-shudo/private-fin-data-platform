...
# Grant permissions for dbt_developer_role on staging/core schemas (read-only)
resource "snowflake_schema_grant" "dbt_developer_staging_usage" {
  schema_name   = snowflake_schema.datawarehouse_staging.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "USAGE"
...
resource "snowflake_schema_grant" "dbt_developer_staging_select_tables" {
  schema_name   = snowflake_schema.datawarehouse_staging.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "SELECT"
  on_future     = true
  future_tables_in_schema = true
  roles         = [snowflake_role.dbt_developer_role.name]
}

resource "snowflake_schema_grant" "dbt_developer_staging_select_views" {
  schema_name   = snowflake_schema.datawarehouse_staging.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "SELECT"
  on_future     = true
  future_views_in_schema = true
  roles         = [snowflake_role.dbt_developer_role.name]
}

resource "snowflake_schema_grant" "dbt_developer_core_usage" {
  schema_name   = snowflake_schema.datawarehouse_core.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "USAGE"
  roles         = [snowflake_role.dbt_developer_role.name]
...
resource "snowflake_schema_grant" "dbt_developer_core_select_tables" {
  schema_name   = snowflake_schema.datawarehouse_core.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "SELECT"
  on_future     = true
...
resource "snowflake_schema_grant" "dbt_developer_core_select_views" {
  schema_name   = snowflake_schema.datawarehouse_core.name
  database_name = snowflake_database.datawarehouse.name
  privilege     = "SELECT"
  on_future     = true
