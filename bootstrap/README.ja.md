...
3. [👉 Step 3: GitHubの環境変数登録](./github/README.md) — 環境（Environments）と初期変数の定義
...
* リポジトリ側に「環境（Environments）」および初期「変数（Variables）」を定義する。変数登録の詳細な流れとタイミングは [docs/operations-runbook.ja.md](../docs/operations-runbook.ja.md) を参照する。
* 本番環境への不正デプロイを防ぐための厳格な保護ルール（自己承認の禁止、管理者バイパスの無効化）を構成する。

---

## 🏁 構築完了の確認

手動Bootstrap（AWS -> Snowflake -> GitHub）が完了したら、[docs/operations-runbook.ja.md](../docs/operations-runbook.ja.md) の実行フローへ進む。

初回構築完了の判定は、runbook 側の完了条件を正本とする。
